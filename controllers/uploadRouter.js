...
module.exports = function (dir, app, db) {
                    ...
                    section => {
                        section.steps.forEach(
                            step => {
                                ...
                                if (step.images_id === null) { // not yet defined, so a new step
    ...
    function processUploads(key4, _id, base, tailnum, numberOfFiles, myFiles, idirs, executionDir, res) {
            ...
            return new Promise((resolve, reject) => {
                Jimp.read(toLarge).then(function (image) {
                    ...
                    () => {
                        ...
                        // progress reporting
                        if (++tab_uploadCount === numberOfFiles) { // finished
                            console.log("***uploadRouter " + uploadCount + " " + numberOfFiles + " : " + key4id);
                            db.collection("progress").findOneAndUpdate(key4query, {
                                $set: {
    ...
    //             fs.renameSync(aFile.path, toLarge);
    //             // fileRefs.push();
    //             // console.log(JSON.stringify(fileRefs, null, 4));
    //             debugLog("converting", index + 1);
    //             Jimp.read(toLarge).then(function (image) {
    //                     let img = image.clone();
    ...
    //             // TODO: announce progress ??
    //             ///////////////////////
    //         });
    //         return promises;
    //     }
    // })
    // debugLog("before returning promises", "");
    // return promises;
    // Promise.all(promises).then(
    //     (values) => {
    //         debugLog("values: " + JSON.stringify(values, null, 4));
    //         res.json({
    //             "count": values.length
    //         });
    //     }
    //     // updateTabImages(key4id, images_id, fileRefs).then(
    //     //     () => {
    //     //         debugLog("final converted", converted);
    //     //         res.json({
    //     //             "count": converted
    //     //         });
    //     //     });
    ...
    function updateTabImages(key4id, fileRefs) { // returns a promise
    ...
    app.get('/clear_progress/:_id', (req, res) => {
        debugLog("clear ", true);
        db.collection("progress").deleteOne(req.params).then(
            (v) => {
                res.json(v);
            },
            (e) => {
            ...
            Object.keys(req.body.fileinfo).map(
                (identifier) => {
                    let p = new Promise((resolve, reject) => {
                        ...
                        col.updateMany(query, update, other).then(
    ...
    app.post('/archiveTabImages', (req, res) => {
        ...
        db.collection('tab_images').updateOne({
            _id: req.body.job_id
        }, {
            $set: {
    ...
    app.post('/set_tabs',
        (req, res) => {
            ...
            // let newStepFiles = newImageIds.map( // the items to be inserted into tab_images collection in array stepFiles
            //     (element) => {
            //         return {
            //             "image_id": element,
            //             "files": []
            ...
            myPromise.then(
                (r) => {
                    ...
                    tab_imagesCollection.updateOne({
                            _id: doc._id
                        }, {
                            // $addToSet: {
                            //     stepFiles: {
                        ...
                        .then(
                            ...
                            (error) => {
                                console.log("Unable to create tab_images document for id = " + doc._id + "\n" + error);
                                res.json({
                                    "error": error
                                });
                            }
    ...
    app.post("/tabUploads", (req, res) => {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            // console.log("files count " + tab_numberOfFiles);
            ...
            inc_promise.then(
                (doc) => {
                    ...
                    Promise.all(
                        tab_processUploads( // somehat different from processUploads for Tools, returns list of promises
                        ...
                        (promises) => {
                            ...
                            Promise.all(promises).then(
                                (values) => {
                                    debugLog("tab processing results: ", JSON.stringify(values, null, 4));
    ...
    app.post("/countTabImages", (req, res) => {
        ...
        db.collection('tab_images').aggregate(
            [{
