...
$(function () {
    ...
    function setup(data) {
        ...
        $("#nav_2").append($('<button id="tabupload_button" class="navButton"><i class="fas fa-upload fa-lg"></i>&nbsp;&nbsp;Upload Images</button>'));
        $("#nav_3").html('');
        $("#nav_4").append($('<button id="run" class="navButton" type="button"><i class="fas fa-bolt fa-lg"></i>&nbsp;&nbsp;Main</button>'));
        ...
        $('.caret').on('click', (e) => { // open/close display below
            let that = $(e.target);
        ...
        function formatSaveErrors(status) {
            ...
            if (tlen > 0) {
                message += tlen + " <b>Tab</b>" + ((tlen !== 1) ? 's</b> have' : '</b> has') + " no defined Section(s).";
                message += '<br/><span class="indent">' + status.tabs.join(", ") + '</span>';
                if (slen > 0) {
                    message += "<br/><br/>";
                }
            }
            if (slen > 0) {
                message += slen + " <b>Section" + ((slen !== 1) ? 's</b> have' : '</b> has') + " no defined Step(s).";
                message += '<br/><span class="indent">' + status.sections.join(", ") + '</span>';
            }
            return message;
        }
        $('#save_button').on('click', // Save button
            (e) => {
                let doc = {
                    _id: key4id,
    ...
    function paintPage(jobDoc) { // tabs is a single document for one job
        let brandNew = false;
        ...
        let emptyTab = li("", 't', unique++, "tabname empty", false, "The Tab Name");
    ...
    function li(name, tag, num, aClass, addCaret, title) {
        if (name === null) { // as yet unsaved/undefined Tab, Section or Step
            ...
            place = '&rarr;&nbsp;Complete this ' + place.charAt(0).toUpperCase() + place.substr(1);
            let input = '<input class="emptyinput" placeholder="' + place + '" type="text" title="' + title + '"/>';
            result += input;
            ...
            if (result.find('.addcircle')) {
                ...
                if (name === "") {
                    ...
                    // $('body').on('keydown', foo);
                    result.find('input').on('click', inputClick).on('dblclick', inputDoubleClick).on("change", showSave);
