...
$(function() {
	...
	const run = async () => {
		...
		const toolData = await Util.getSheetTags(key5, "Tools");
...
function toolsTable(machineSpecs, toolData) {
	let aToolDiv = $(
		`<div class="tooldiv">${right_caret}<h2 class="toollink">Tools</t2></div>`
	);
	...
	["", "Position<br/>#", "Offset<br/>#", "Function", "Type"].forEach(head =>
...
function doRows(specs, turret, spindle, table, haves) {
	...
	for (var t = lowT, s = lowS; t <= highT; t++, s++) {
		...
		if (haves[link] !== undefined) {
			...
			var a =
				'<a target="_self" href="/tabs/2.html#' +
				link +
				'">' +
				"&#9675;" +
				"</a>";
...
function drawMainTab(tab, tab_index) {
	let aTabOuter = $(
		`<div class="tabouter"><div class="tabdiv">${right_caret}<h2 class="tablink">Tab: ${
			tab.tabName
		}</h2></div></div>`
	);
