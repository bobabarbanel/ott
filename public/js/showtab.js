...
$(function () { // onload
    ...
    Util.setUpTabs(TV.key4id, tabName).then(
        (tabs) => {
            getTabImages().then( // gets only non-archived images
                (stepFiles) => { // an array of fileRefs, each with images_id attribute; may have no key/value for steps without images
...
function paintPage(tab, stepFiles) {
    ...
    tab.sections.forEach(
        (section, sect_index) => {
            ...
            let sectDiv = $(`<div id="${section_target}" class="headtext"><span class="sectionlabel">&nbsp;Section:&nbsp;</span>${section.sectionName}</div>`);
            ...
            if (section.steps.lemgth === 0) {
                ...
                section.steps.forEach(
                    (step, step_index) => {
                        ...
                        let stepDiv = $(`<div class="step pic" id="pic${step_target}" images_id="${step.images_id}"></div>`);
                        ...
                        if (curLG !== null) { // have a group?
                            if (curLG.getStart() !== curLG.getStop()) { // ignore groups with no images
                                curLG.setNext(new LinkGroup(curLG, null, TV.maxImageShowing, null, step_target));
                                curLG = curLG.getNext();
                            } else {
                                curLG.setLink("pic" + step_target);
                            }
                        } else { // initialize
                            curLG = new LinkGroup(null, null, TV.maxImageShowing, null, step_target);
                            LG = curLG;
                        }
                        ...
                        if (fileRefs !== undefined && fileRefs.length > 0) {
                            ...
                            stepSpan = $('<span class="stepSpan"/>').html(TV.SPACE + TV.SPACE2 + step.stepName);
                            stepDiv.append(button, stepSpan);
                            fileRefs.forEach(
                                (fileRef, index) => {
                                    let small = fileRef.dir.replace('/' + pageName + '/',
                                        '/' + pageName + '_small/');
                                    let large = fileRef.dir.replace('/' + pageName + '/',
                                        '/' + pageName + '_large/');
                            ...
                            stepSpan = $('<span class="stepSpan"/>').html(TV.SPACE + TV.SPACE2 + step.stepName + " - no images");
    ...
    let dash = /^#section_/;
    let prev = '';
    let icon = '<span><i class="fas fa-tasks" style="color:yellow"></i>' + TV.SPACE2 + '</span>';
    ...
    links.forEach(
        link => {
            if (dash.test(link[0])) { // start (possibly end) ul
                ...
                let line = '<div class="floatlink"><div class="floattag">' + link[1] + '</div>';
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            .fail((request, status, error) => {
                alert("setArchiveForTabImages failure: " + status + " " +
