...
const kindOf = (arg) => arg === null ? "null" : typeof arg === "undefined" ? "undefined" : /^\[object (.*)\]$/.exec(Object.prototype.toString.call(arg))[1];
const isFunction = (arg) => "Function".toLowerCase() === kindOf(arg).toLowerCase();
const isRegExp = (arg) => "RegExp".toLowerCase() === kindOf(arg).toLowerCase();
...
function find(path, opt) {
    ...
    if (options.filter) {
        if (isRegExp(options.filter)) {
            result = result.filter((item) => options.filter.test(item.path));
        } else if (isFunction(options.filter)) {
            result = result.filter((item) => options.filter(item.path, item.stats));
        }
    }
    return result;
}
