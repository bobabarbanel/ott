...
function getOptions(opt) {
    ...
    options.ignoreAccessError = "ignoreAccessError" in options ? options.ignoreAccessError : false;
    let stats2 = new options.fs.Stats();
    options.showStats = Array.isArray(options.stats) ? options.stats : Object.getOwnPropertyNames(stats2).concat(Object.getOwnPropertyNames(Object.getPrototypeOf(stats2)));
    return options;
}
...
function getResult(options, path, stats) {
    ...
    return options.back === 2 ? miniStats : options.back === 1 ? path : {path, stat: miniStats};
}
