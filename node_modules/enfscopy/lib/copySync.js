...
class CopyFiles {
    ...
    loadItems() {
        ...
        return this.loadStatistics(items);
    }


    loadStatistics(items) {
        this.statistics.items = items.length;
        this.statistics.size = items.map((item) => item.stat.isFile() ? item.stat.size : 0).reduce((acc, val) => acc + val, 0);
...
function copy(src, dst, opt) {
    ...
    return (new CopyFiles(src, dst, options)).start();
}
